...
$Expand_image      = $config->get('slideshow_settings', 'Expand_image');
   ...
   {  
      ...
      $Expand_image=$_POST['Expand_image'];
      if(isset($Expand_image) === false){$Expand_image = "False";}else{$Expand_image="True";}
      ...
      $config->set('startup_setting', 'Ignore_USB_Stick', $Ignore_USB_Stick);
      $config->set('slideshow_settings', 'Expand_image', $Expand_image);
      $config->save();
      header("Refresh:0");
   }
?>
...
<br>
      ...
      Skip USB-Stick check
      <br>
      <input name="Expand_image" type="checkbox" value="1" <?php if ($Expand_image == 1){?> checked="checked" <?php } ?>/>
      Expand Image
      <br>
      <br>
      <hr>
